...
export const FOE_CONDITIONS = Object.freeze(
  [
    ...
    { key: "stunned", label: C.stunned, tone: "good", fields: ["stunned"], when: (f) => !!f.stunned, rounds: none },
    // Dirty Trick (blind + blindFor 2, ticked down in foeTurn) and the Blind spell (blind alone).
    { key: "blind", label: C.blind, tone: "good", fields: ["blind", "blindFor"], when: (f) => !!f.blind || posInt(f.blindFor) !== null, rounds: (f) => posInt(f.blindFor) },
    // engine/abilities.js applyHamstring: half damage for the rest of the fight.
    { key: "hamstrung", label: C.hamstrung, tone: "good", fields: ["hamstrung"], when: (f) => !!f.hamstrung, rounds: none },
    // engine/abilities.js applyMark: +2 on every hero strike at this foe.
    { key: "marked", label: C.marked, tone: "good", fields: ["marked"], when: (f) => !!f.marked, rounds: none },
    // Sleep/Doze spells, Sing, items: a countdown decremented each foe turn.
    { key: "asleep", label: C.asleep, tone: "good", fields: ["asleep"], when: (f) => f.asleep === true || posInt(f.asleep) !== null, rounds: (f) => posInt(f.asleep) },
    // Freeze/Ice/Petrify. The engine clears it on a foe that stands back up.
    { key: "frozen", label: C.frozen, tone: "good", fields: ["frozen"], when: (f) => !!f.frozen, rounds: none },
    // The Acid spell's `{ rounds, dmg }` record.
    { key: "acid", label: C.acid, tone: "good", fields: ["acid"], when: (f) => !!f.acid && posInt(f.acid.rounds) !== null, rounds: (f) => posInt(f.acid.rounds) },
    // The shared `{ left, dmg, by }` DOT: Poisoned Edge, and Ice (by "ice").
    {
      key: "dot", label: C.poison, tone: "good", fields: ["dot"],
      labelFor: (f) => (f.dot.by === "ice" ? C.ice : C.poison),
      when: (f) => !!f.dot && typeof f.dot === "object" && posInt(f.dot.left) !== null,
      rounds: (f) => posInt(f.dot.left),
    },
    { key: "stupid", label: C.stupid, tone: "good", fields: ["stupid"], when: (f) => !!f.stupid, rounds: none },
    { key: "shrunk", label: C.shrunk, tone: "good", fields: ["shrunk"], when: (f) => !!f.shrunk, rounds: none },
    { key: "fixated", label: C.fixated, tone: "good", fields: ["fixated"], when: (f) => !!f.fixated, rounds: none },
    // The one foe BUFF: it swings twice.
    { key: "frenzied", label: C.frenzied, tone: "bad", fields: ["frenzied"], when: (f) => !!f.frenzied, rounds: none },
    ...
    { key: "weakened", label: C.weakened, tone: "good", fields: ["weakened"], when: (f, s) => !!(s && s.combat && s.combat.weakened), rounds: (f, s) => weakenLeft(s) },
 ...
 * `{ key, label, tone, rounds, text }`, in table order, for a LIVE foe.
 * `text` is "label · rounds" when rounds is set, else the label.
...
export function foeConditionChips(foe, state) {
  ...
  for (const e of FOE_CONDITIONS) {
    ...
    out.push(Object.freeze({ key: e.key, label, tone: e.tone, rounds, text: rounds !== null ? `${label} · ${rounds}` : label }));
  }
