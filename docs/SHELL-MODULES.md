...
| __mzTabs | mazeworld.html (module) | mazeworld.html (classic: paint() — one call per tab surface; renderEncounter() — the store branch) | The tab modules' render functions, one frozen object (gear + hero today; Plan 05 adds store) — the __mzControls/__mzTables precedent for a module-assigned, classic-read bridge. |
