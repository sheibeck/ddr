...
| __mzPartySprite | mazeworld.html (module) | mazeworld.html (classic: partyShown — the displayed point a step glide is heading toward)<br>mazeworld.html (classic: positionPartySprite — box/pose/frame, in lockstep with positionCanvas())<br>mazeworld.html (module: settleAllMotion — finish()) | Bridges the pure src/browser/partySprite.js marker controller (Phase 59, ANIM-01/02) so the classic placement code reads one lockstep box and one step glide, never a second copy of the camera math. Plan 59-04 adds the classic glideParty (stepTo) to this list. |
