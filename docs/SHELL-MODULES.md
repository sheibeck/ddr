...
| __mzHudBands | mazeworld.html (module) | mazeworld.html (classic: paint — the HUD identity line and the fixed-width counter slots) | Bridges the pure src/browser/hudBands.js identityLine/counterSlots formatters (Phase 57, LAYOUT-05) so paint() renders band 1's identity line and band 2's fixed-width counters from ONE engine-agnostic source. |
