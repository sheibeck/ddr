...
Resolved by Phase 57 plan 04 (LAYOUT-06): the counter-driven vignette (fed `mapViewRadius`, not `revealRadius` — see the correction below), the DARK chip's rewritten tap card, and named `darknessFell`/`darknessDispelled` rail cards. **Correction found during planning/execution:** this todo's premise ("nothing reads the counter") was wrong at HEAD — Phase 41 (TERR-03) had already wired a render-window filter (`mapViewRadius`/`inViewWindow`) that collapses the map to a 3x3 window whenever `c.darkFor > 0`, with no waiver open. The real gap was that nothing on screen explained why the window closed or that a torch/Amulet/Night Vision waiver was holding it open while the counter kept ticking — which is the actual 2026-09-21 device report. 57-04 built the vignette as legibility on top of the already-correct filter and added the waiver-naming clause; it did not "fix" a missing read.
