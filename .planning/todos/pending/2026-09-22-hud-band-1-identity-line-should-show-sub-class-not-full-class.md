...
**Open question the user has not answered yet:** keep the race in the line (e.g. "Dwarf Pickpocket · Lvl 3") or drop it too (e.g. "Pickpocket · Lvl 3")? Get a ruling before implementing.

## Solution

Once the race-keep-or-drop question is answered: change `identityParts`'s `line` composition so it uses only the sub-class (falling back to the base class when a character has no sub, same as today's omission rule for a falsy `c.sub`) instead of joining race + class + "(sub)". Keep the level suffix and separators unchanged. Verify against `test/unit/hudBands.test.js`'s existing identity-line pinning tests (all seven will need updating for the new expected strings) and re-check the mock's never-truncated name span vs. the dimmed/ellipsis-truncating line span still make sense with the shorter text. Post-milestone UI quick task, presentation-only.
