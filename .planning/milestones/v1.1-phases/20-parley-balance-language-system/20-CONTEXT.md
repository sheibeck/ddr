...
- **D-17:** `mazeworld.html` keeps its OWN duplicate `canParley()` (~L4200, reads `S.combat`/`S.c` directly; live code per the DR8 comment) that drives the "5 · Parley" button. It MUST be updated in lockstep with `engine/combat.js#canParley` (fluency tiers, Magical at fluency 2, `parleyTried` hides the button) or the button and the engine will disagree. The classic `parley()` beside it is dead code — updating it is optional hygiene, not gating.
- **D-18:** The parley fixture scenario is replayed by TWO tests: `test/parity/combat-parity.test.js` (local `comparable()`) AND `test/parity/full-suite.test.js` (shared `combatComparable` in `test/parity/harness/comparables.js`). The D-13 scenario-scoped stripper must be applied at BOTH call sites (same named helper, exported from the harness), or the phase gate stays red.
- **D-19:** No save-migration work for `C.parleyTried` / `C.parleyInsulted`: `validateSave` and `rehydrate` already null `state.combat` unconditionally on load (confirmed by direct read). The flags still need the comparables carve-out (D-13) but no `saveState.js` change.
- **D-20:** D-06's "+1 to every foe to-hit roll" is applied as **`need += 1`** (the foe's target number), at BOTH `foeTurn` call sites (hero swing and member swing) — matching the file's existing precedent (`Agility`, `foeToHitPenalty` adjust `need`, never the roll). Zero extra draws.
- **D-21:** Seed-303 before/after (computed from the locked formulas; RESEARCH.md has the draw-by-draw table): need 19 → 17, sp 13 → 7, gold 200 → 0 — draw positions unchanged (d20=2, d6=5, d6=5); under `d6 === 6` the wilmst check (5) simply no longer fires, so the 4th draw stops. The planner must have the executor re-measure these against the landed code before writing the FIXTURE-INVENTORY table.
