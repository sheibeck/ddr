...
- **D-17:** `resistRoll(rng, intel)` lives in `engine/derived.js` (a dependency-free leaf), NOT `engine/magic.js` — `magic.js` already imports `combat.js`, and `combat.js` must import `foeAbilities.js`, so homing it in `magic.js` would create an import cycle. `magic.js` imports it from `derived.js`. Supersedes D-07's file location only; the single-d20 draw-neutral refactor stands.
- **D-18:** Foe bolts/drains have NO to-hit roll — Intelligence resistance (plus ward/armor/Hardiness for bolts, ward for drains) is the counterplay, and the D-03 ~50%-HP cap is the fairness valve. `applyFoeDamageToPlayer` gains two ADDITIVE options: `ignoresArmor` (drain skips the soak d20) and an `applied` return field (drain heals the foe by the amount that actually landed); existing callers are unaffected (draw-neutral).
- **D-19:** The Spectre pursuit free strike applies to EVERY successful flee exit (all flee-success branches), whenever a live `pursues` foe is present.
- **D-20:** `every: N` cooldowns start armed at combat start so Drake's first breath fires on round 4, then 8, 12… (`f.cd[id]` initialised to N on first evaluation; decremented each foe turn; fires when it reaches 0 and resets to N).
- **D-21:** `conditionsOf`'s new `foeEffect` chip needs a label entry in the existing chip copy map (`CONDITION_COPY` in `mazeworld.html` `paintConditions()`, and any view-model label map) — "Weakened" / "Dazed" in voice. This is a label row in the existing chip row, not a new screen, so it is in scope; the engine test alone would not catch its absence.
