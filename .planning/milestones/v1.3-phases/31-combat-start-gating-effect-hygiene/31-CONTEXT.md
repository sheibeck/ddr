...
- New mechanic "Afraid": trigger unchanged (type-matched phobia / Darkness in the dark / Death near death; Hardiness `rng.d(2)` still halves the odds — same draw, same site). Effect: `combat.afraid = 2` rounds; while afraid every attack roll the player makes (strike to-hit, and spell to-hit where one exists) is HARDER by 3 — the game's to-hit is a LOW range (hit on d20 ≤ toHit, "1–N"), so the penalty SHRINKS the target number (`Math.max(1, toHit - 3)`) or equivalently adds 3 to the roll side; NEVER `roll − 3` (user correction 2026-09-16) — and the player's damage is halved (Math.ceil, min 1); the player may act normally otherwise. ZERO new rng draws (no shake-off roll). Counts down once per round with the other Phase 31 round-based effects; clears at `endCombat`; condition chip "Afraid · N rds".
