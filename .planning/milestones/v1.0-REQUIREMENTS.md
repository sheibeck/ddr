# Requirements Archive: v1.0 Delve, Die, Repeat — Android build & internal testing
...
- [x] **VOX-01**: A data-driven voice system generates the game's sarcastic, dark-but-family-friendly copy (death epitaphs, event/Oracle log lines, item flavor) keyed to structured game events (death cause, class/race, depth)
- [x] **VOX-02**: A batch tone/rating QA pass validates the procedural flavor-text combinations stay family-friendly (no profanity/gore) before rating is finalized
- [x] **VOX-03**: The player can review a graveyard / run-history screen showing past adventurers and their epitaphs
...
- [x] **STR-05**: A signed Android App Bundle is produced via Play App Signing and validated through the Play internal testing track on real devices
...
- [x] **PARTY-01**: The player can recruit an NPC "Joiner" — a fully-rolled adventurer (its own class/race/level/gear/HP) — through the Joiner encounter's accept/decline offer (declining leaves them behind, per rulebook canon)
- [x] **PARTY-02**: A recruited joiner persists in the run's saved state (a party roster) and survives app close/reopen; pre-existing saves default to an empty party with no data loss
- [x] **PARTY-03**: In combat, the joiner auto-acts each round using its own strike math (no per-member micro-management), alongside the hero
- [x] **PARTY-04**: Foes can target either the hero or the joiner, and the joiner has its own HP that takes damage and can reach 0
- [x] **PARTY-05**: A joiner beaten to 0 HP permanently dies and departs the run (with an on-tone line) and NEVER triggers the hero's death/run-end; a surviving joiner eventually leaves on its own after a while
- [x] **PARTY-06**: XP from kills splits among all participants who fought (a joiner reduces the hero's per-kill share, per canon); the hero holds all wilmst/loot (joiners carry no separate inventory or progression in v1)
- [x] **PARTY-07**: A party rail shows the active joiner's identity, HP, and status during play (shown only when a joiner is present, legible in portrait on a phone)
- [x] **PARTY-08**: The party is capped at ONE joiner in v1, with the underlying model built to support additional members later without a save break
- [x] **PARTY-09**: Joiner flavor (join / kill / downed / leave) is sarcastic and family-friendly, reusing the game's voice
...
- [x] **ECON-01**: Every character starts with a class-derived bag whose capacity caps item slots (4/6/8/10), a wilmst cap, and a rations cap; every carried (non-equipped) item takes a slot
- [x] **ECON-02**: Bags/carry state persists and rehydrates losslessly; pre-existing saves migrate to a default bag with no data loss
- [x] **ECON-03**: On finding an item, the player chooses whether to take it (no auto-grab)
- [x] **ECON-04**: When the bag is full, the player chooses which items to keep and which to drop
- [x] **ECON-05**: The player can manually equip items from the bag — including gear inferior to what's worn — while class/subclass/race equip restrictions stay enforced (can't equip what you can't use)
- [x] **ECON-06**: The player can sell any carried item at a store, which lists all carried gear with sell prices
- [x] **ECON-07**: Combat-usable carried items are usable from the combat bar (E2), sharing one carried-item list component with the store-sell list
- [x] **ECON-08**: Every item either has a working, wired effect or is deliberately retired; currently-inert item effects (Helm of Knowledge/`tongue`, Cloaks of Healing/Regeneration/Strength, Pendant of Fortitude, Amulet of Light `light`, Amulet of Stone's 4-target) are fixed; treasure items carry a base value for pricing
- [x] **ECON-09**: Over-generous/obviously-off economy numbers get a conservative first-pass fix (the +3000 wilmst red-dot reward depth-scaled with NO new rng draw; egregious costs) — deep balance deferred to on-device playtesting
- [x] **ECON-10**: The three bag caps (slots/wilmst/rations) are set to sensible v1 values, all left as documented tuning knobs
...
| UX-06 | Phase 4: Mobile Presentation, Controls & Onboarding | Pending — tutorial (04-10) deliberately deferred until the UI settles (user directive) |
...
| STR-05 | Phase 6: Google Play Compliance & Launch | Complete — signed AAB on the Play internal-testing track, testers on real devices (2026-09-10) |
| STR-06 | Phase 6: Google Play Compliance & Launch | Pending |
| PARTY-01 | Phase 9: Joiner Acquisition + Voice | Complete |
| PARTY-02 | Phase 7: Party Model + Save Migration + Parity Carve-outs | Complete |
| PARTY-03 | Phase 8: Party Combat | Complete |
| PARTY-04 | Phase 8: Party Combat | Complete |
| PARTY-05 | Phase 8: Party Combat | Complete |
| PARTY-06 | Phase 8: Party Combat | Complete |
| PARTY-07 | Phase 10: Party UI | Complete |
| PARTY-08 | Phase 7: Party Model + Save Migration + Parity Carve-outs | Complete |
| PARTY-09 | Phase 9: Joiner Acquisition + Voice | Complete |
| PARTY-10 | Phase 11: Party Balance (consolidated) | Deferred — party-local knobs done; the SINGLE consolidated difficulty retune runs in the Monster Balancing milestone |
