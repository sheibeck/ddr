...
- [ ] **RULES-09**: A Pilfer can use magic items under the normal rules. The heal-only refusal is gone. Instead, every time a Pilfer uses a magic item (use-activated jewelry, cloaks and staves) it rolls a d20, and on a 1 the use fails, the item explodes for d10 damage to the Pilfer (armor does not soak it), and it turns to dust. Scrolls are not magic items for this rule. The player is told so. Tools never fumble. The fumble draw comes from a derived rng stream. The Pilfer blurb states both sides. (user, 2026-09-24; todo 2026-09-24 pilfer-bad-becomes-fumbling)
