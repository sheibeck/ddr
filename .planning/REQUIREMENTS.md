...
- [x] **DEAD-04**: `winGame()` / `state.won` and their parity carve-out are removed (RUN-04 made them unreachable); the harness no longer strips a `won` field
- [x] **DEAD-05**: `src/browser/tutorial.js` is either deleted (with a note that UX-06 rebuilds on the modular shell) or parked with a header stating why it is unreferenced — decided in the phase, not left ambiguous
