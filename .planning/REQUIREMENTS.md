...
- [x] **CANON-02**: Spectre pursues a fleeing player (reuses the existing flee resolution path); Drudge never melees; Drake's breath is gated by an `every`-N cooldown (mirrors the item-cooldown pattern)
...
- [x] **TUNE-04** (DEFERRED — DR verdict tune-again 2026-09-14; retune moved to a post-cleanup/class-fixes milestone): Final constants are signed off by a human on-device DR round at depth 20–50+ against caster foes — harness numbers are a sanity floor, not the exit criterion (UAT deferred to milestone end)
