...
- [ ] **Phase 47: Shell Modularisation** - The Gear tab, Hero tab and Store screen render from `src/browser/gearTab.js` / `heroTab.js` / `storeScreen.js` with source-pin tests; `mazeworld.html` is a mount point (5,621 lines, re-baselined from the < 5,000 target by user ruling) with the `window.__mz*` bridge listed in one place
...
**Success Criteria** (what must be TRUE):
  ...
  2. `wc -l mazeworld.html` < 5,000. **Re-baselined 2026-09-19 (user ruling):** the three carves and their private helpers landed the shell at 5,621 (−718); everything left is Map/Combat/Rail/Oracle/Graves code the phase ground rules keep in the shell, comments (Phase 48), and markup/CSS. Accepted at 5,621 — see `docs/SHELL-MODULES.md#Line budget`.
...
- [x] 47-01-PLAN.md — DOM-snapshot harness (recording document + node:vm classic-script sandbox with the bridge twin) and the seven committed BEFORE fixtures; standing 3-screen smoke with the store idempotency truth

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 47-02-PLAN.md — `src/browser/bridge.js` registry + `bridge-registry.test.js` (comment-stripped set-equality, fail-first teeth), `tools/bridge-doc.mjs`, `docs/SHELL-MODULES.md` with the generated `## Module bridge`; dead `__mzBags` deleted

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 47-03-PLAN.md — `src/browser/gearTab.js` (Gear view models, `renderCarriedList` + drop/swap confirms, `renderGearTab`); `paint()` mounts through `window.__mzTabs.gear`; `tabDeps()`; six dead bridges + classic helpers deleted; pins re-homed; `gearTab.test.js`
