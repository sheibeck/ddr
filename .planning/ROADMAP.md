...
- [ ] 54-01-PLAN.md — The four bands verbatim + numeric targets + the structural bound (reach ≥5 fixed by floors 1–4 — why a Filter rung exists, USER RULING A) in the ledger H3 (BAND-01); band-curve scaffolding in `engine/difficulty.js` at identity values (`WALL_*` / `BREAKAWAY_*` / `ENDGAME_CANON_FROM_DEPTH`, `bandFoePowerFor`, literal 1 from 16) with the curve pinned byte-identical to Phase 53's (floor-1 and 16+ literals never re-pinned; Filter 2..4 and band 5..15 pins re-pinnable per rung; Wall-steps-up; draw-free); the always-on `Four-band readout` block in `tune-difficulty` (reach ≥16 / band share / per-band causes — bot policy untouched); the BEFORE `--start-depth=20` slice on the untouched engine; the BAND-02 measured-zero guard

**Wave 2** *(blocked on Wave 1 completion)*

- [ ] 54-02-PLAN.md — The iteration ladder (cap 4 rungs; rung 1 = Wall 0.85 → 0.95 / Breakaway 0.95 → 1.0 / literal 1 from 16; if the solo median is still ≤ 4, rung 2 is a Filter rung on the EXISTING Phase 27 dials `FOE_GRACE_AT_2` (≥ 0.35) / `HAZARD_SCALE_AT_START` (≥ 0.3) per USER RULING A — floor 1 exact identity, no new floor 1–4 dial; ≤ 0.05 / one-notch moves after; hazard / ability band by the previous rung's per-band causes) — each rung a green constants commit, then the FULL four readouts on that commit (solo / `--party` / class smoke / `--start-depth=20` byte-identical) plus the floor-1 proof (scan Part B diff empty, floor-1 literal pin), committed verbatim under `#### Rung N`; then parity measured on the final curve and declared MOVED SET (0) (FIXTURE-INVENTORY Phase 54 + guard), the change table (one row per constant incl. the two Filter dials), the miss table (solo median a live target — recorded as a miss only if still outside 5–7), the depth-20 identity proof, `v17-p54-after-smoke.json` (BAND-02, BAND-03, BAND-01)

**Wave 3** *(blocked on Wave 2 completion)*

- [ ] 54-03-PLAN.md — Damage-curve audit re-run on the AFTER curve (fold verified: `dmgBonusForBand` reads `foeDmgBonusFor(T, difficultyCurve(d))`; locked v17-p51 yardstick; both rules) + BESTIARY-REBALANCE Phase 54 addendum (cell count 65 → AFTER, every remaining row re-dispositioned, in-place retune only for a still-≥100 % one-shot at its own tier's band — the Drake, 2d10+4 → 2d8+2 per USER RULING B, `drakeBreath` untouched — declared end-to-end with post-retune readouts) + the roster decision per creature (Herman / Drarl / Vampire / Djinni record `stays` from the AFTER audit + the forced-20 slice; untaken forced-20 rungs named as the deliberate Endgame shape; parity declared) + the phase roll-up SUMMARY (TUNE-08, BAND-02, BAND-03)
