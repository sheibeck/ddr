...
- 2026-09-24 — **Thieves read scrolls at 50%** (user, added to v2.1 as RULES-10, Phase 75): any Thief may attempt a scroll, 50% success, scroll consumed either way — `todos/pending/2026-09-24-thieves-read-scrolls-at-50-percent-scroll-always-consumed.md`
...
- 2026-09-23 — **UI tap sound plays when scrolling over a button** (user, device): `playUiTap()` rides a capture `pointerdown` (`mazeworld.html:5842`), so a scroll that starts on a button clicks. Fix: keep `unlockSfx()` on pointerdown, move the tap to a capture `click` listener (no click after a scroll/pointercancel); skip disabled buttons, no double sound on buttons with their own clip. Shell-only quick task — `todos/pending/2026-09-23-ui-tap-sound-plays-when-scrolling-over-a-button.md`
- 2026-09-23 — **Narrate a destroyed armor piece when new armor replaces it** (user, device: "old armor dropped, not bagged" — it was destroyed, rule correct, but silent): `equipItem` / `takeLoot(equip)` / `takeItem` drop a destroyed worn piece (`wornArmorItem` → null) with no trace. Fix: additive `destroyed`/`discarded` payload on the existing event + a voiced line, mirroring `unequipSlot`'s destroyed branch; optional sheet note. Engine+narration quick task, zero rng — `todos/pending/2026-09-23-narrate-a-destroyed-armor-piece-when-new-armor-replaces-it.md`
...
- AAB: 2.0.0/vc10 built 2026-09-24 at the user's request and tagged v2.0.0-play10 (local, not pushed): Phases 65–71 + quick 260924-g8m + the user's re-recorded walk1–3 clips + the live Play Games IDs; sha256 732b96a6…ec214, upload key 50:45:7F…56:DD. The user uploads it by hand; never upload. (vc9 / v2.0.0-play9 is superseded.)
...
- [Phase ?]: Deferred a combat round's clips via dispatchWithNarration(action, opts={}) pushing onto a caller-owned opts.cues array, keeping the return contract unchanged for its other 8 callers
- [Phase ?]: Pinned the paint()/draw() modularity proof's SHA-256 against PRE58 (Phase 57 close, ab3fca9) rather than BASE_58, since BASE_58 postdates Phase 57's own legitimate HUD edits to those functions
