...
- **SUMMONER HEALING WEAKNESS (user, 2026-09-25; todo `2026-09-25-summoner-weakness-healing-spells-half-as-effective`):** healing-school spells CAST BY a Summoner restore half (floor, min 1), on any target (self, members, allies). Potions and others' heals on the Summoner are unaffected. The summon backfire stays. Implement it as a chart data flag (e.g. `healMul: 0.5` on the Summoner row), not a name check. There's no new rng. Measure, declare and regenerate the moved fixtures. The Oracle shows the halved amount.
