...
|---|---|---|
| Natural bot median death depth | 3 (mean 3.08) | **Bot: median 4, pooled reach ≥ 5 at ≥ 25 %** — the measured ceiling of the sanctioned levers (planner calibration 2026-09-15: every rung of the widened ladder, floor-1 grace included, tops out at bot median 4 / reach-5 ≈ 35 %; what remains at floors 2–4 is canon tier-2/3 combat, starvation and falls, which the bot plays badly). **Human expectation: median 5–6**, judged by the DR round, not by the proxy. (User decision 2026-09-15, third round: "Bot median 4, human 5–6".) |
...
3. **Widened early-floor levers (user decision 2026-09-15, after the planner's calibration showed the bot median cannot leave 3 with dials + Dante alone).** Measured kill table (143 cells × 4 seeds): 20 % die on floor 1, 66 % by floor 3, 85 % by floor 4; floors 2–4 deaths are a wide spread — canon tier-2/3 foes (Poltergeist, Gremlin, Drekk, Philly, Google, China Wolf, Zit), traps (~12 %), wall-falls (~8 %), starvation in the dark (~10 %). A bot median of 5–6 needs survival through floor 4 to go from ~15 % to ≥ 50 %, so the retune may ALSO use, as deliberate canon deviations recorded in the ledger:
   - **Foe grace at floors 2–4:** foe wp/damage scaled BELOW identity at floors 2–4 (e.g. ×0.75 at floor 2 rising to ×1.0 by floor 5) through the existing `difficultyCurve` (`foePower` < 1 permitted only for depths 2–4; floor 1 stays exactly 1.0). Parity: only the movement fixture that descends to floor 2 (seed 256) diverges — declared.
   - **Trap and wall-fall damage ramp:** trap and fall damage scaled by depth (e.g. ×0.5 at floors 1–3, canon by floor 5) as a new draw-free curve field consumed by `engine/movement.js` (traps, climbs/falls, gorges). Parity: any floor-1 fixture that springs a trap or falls diverges — declared, machine-checked; if more than a couple of scenarios diverge, start the ramp at floor 2 instead (floor 1 canon) and record why.
   - **Starvation relief:** darkness arrives later (first dark blobs from floor 4) and/or starting rations +1–2 in chargen. Darkness-only is parity-clean; a rations change diverges the chargen fixtures (declared) — prefer darkness first, rations only if the median still misses.
   - **Floor 1 foes stay canon** (orchestrator decision under the user's "you decide"): the Dante demotion already removes ~56 % of floor-1 deaths, and a floor-1 grace would diverge every combat/magic/encounter fixture. LAST RESORT ONLY: if calibration with everything above still leaves the bot median < 5, the foe grace may extend to floor 1 (e.g. ×0.85) with declared divergences and a ledger note that the fidelity contract now carries broad early-floor exceptions.
   - **Ladder cap (user decision 2026-09-15, third round):** land the PARITY-CLEAN set only — Dante Form C, foe grace at floors 2–4 (×0.5 permitted, floor 1 exactly 1.0), darkness HELD through floor 3 (not the from-floor-4 form, which diverges movement seed 256), trap/fall hazard ramp FROM FLOOR 2 (floor 1 canon). The rations rung, the hazard-from-floor-1 rung, the darkness-from-4 rung and the floor-1 grace rung are NOT climbed — recorded in the ledger as "available, not taken" with the calibration numbers, so the fidelity contract keeps exactly one declared divergence (seed 303).
   - Calibration discipline: the planner/executor tries the smallest set that reaches median ≥ 5 (target 5–6), in this order — Dante → foe grace 2–4 → darkness later → trap/fall ramp → rations → floor-1 grace — measuring each step with the smoke; every landed deviation gets a `DELIBERATE RULES CHANGE (Phase 27, 2026-09-15, TUNE-06)` comment, a change-table row, and its divergence record. Parity stays byte-identical for every scenario not named in a record; `prototype-master.js.txt` is never edited.
4. **Nothing else.** No class-specific change (Phase 26's revisit list is empty), no upkeep/economy dial unless a counterweight trigger from `docs/DIFFICULTY-RETUNE.md`'s existing rules fires (record it if it does), no change to `engine/character.js`, spells, or items.
