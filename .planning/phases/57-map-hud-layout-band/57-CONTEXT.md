...
3. **LAYOUT-06's premise was factually wrong — this is the material one.** This CONTEXT says *"`draw()` dims only `tile.dark` cells and nothing reads the counter."* That is false at HEAD. Phase 41 (TERR-03) already ships a render-window filter: `engine/derived.js` `mapViewRadius`/`inViewWindow` collapse the map to a 3×3 window whenever `inDark(state)` is true, and `inDark` is `tileDark || c.darkFor > 0`. `mazeworld.html:1955` applies it in `draw()`. Verified empirically:
   ...
   **USER RULING 2026-09-22 (a): make the waiver visible.** While `c.darkFor` runs with a waiver open, the DARK chip names what is holding the dark back and its tap card says which one. A darkness counter beside unrestricted vision with no explanation IS the reported bug.

   **USER RULING 2026-09-22 (b): the vignette follows `mapViewRadius`, not `revealRadius`.** The two disagree under a waiver (a lit torch leaves `revealRadius` at 1 while `mapViewRadius` is `Infinity`), and the vignette exists to explain what the map is RENDERING. Driving it off `revealRadius` would dim the screen while the map showed everything — the very contradiction this requirement removes.

   **USER RULING 2026-09-22 (c): unifying the two mechanisms is backlog 999.8, not this phase.** The split is canon-vs-invention, not two implementations of one idea: `revealRadius` ports the 1994 `reveal()` line verbatim and is waived only by Night Vision; `mapViewRadius` is Phase 41's own render filter, waived by Night Vision, the Amulet and a lit torch. So a torch reopens your whole explored map but does not help you see one square further as you walk — backwards from what a torch should do. Unifying means widening `revealRadius`, which changes which cells enter `seen`; `seen` is serialized, so parity fixtures move, which this milestone's presentation gate forbids. Captured as **backlog 999.8** with the measured divergence, the shape of the fix, and the greenfield declare-and-regenerate treatment it needs. Phase 57 makes today's behaviour legible; 999.8 makes it consistent.
