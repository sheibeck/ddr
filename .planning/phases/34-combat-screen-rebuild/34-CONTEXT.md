...
### Fight! gate (CSCR-06) — user decision: fold in
- `combat.pending`: header `ROUND —`/`N STANDING`, foes + YOUR LOT render, the log shows the encounter line, the action area is ONE full-width `FIGHT!` button (mock's over-panel button style, gold) — nothing else offered (Phase 31). Guarded.
