...
test("paint() contains exactly one window.__mzTabs.hero(...) mount call, before the gear mount, and no Hero-tab id/doss literal", () => {
  ...
  assert.equal(
    (region.match(/"(s-level|s-name|s-tag|s-wp|s-wpmax|s-wpfill|s-die|s-hit|s-dmg|s-arm|s-int|s-sp|s-next|s-cost|s-rations|s-rations-n|s-trait|s-vp|s-skills|s-abilities|doss-who|doss|btn-abandon-character)"/g) || []).length,
    0,
