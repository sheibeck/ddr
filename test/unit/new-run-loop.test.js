...
// best-depth (the dev-loop localStorage stand-in Phase 2/SAV-04 relocates to
...
test("startNewRun() from a dead run yields a fresh, valid, dice-rolled run in a single call", () => {
...
test("startNewRun() records the ended run's deepest floor as the best-depth", () => {
  withFakeLocalStorage(() => {
    ...
    assert.equal(getBest(), 12, "the ended run's deepest floor (its score) was recorded as the best-depth");
  });
});

test("startNewRun() with an explicit seed produces that exact deterministic run", () => {
  withFakeLocalStorage(() => {
    const dying = initRun(1);
    dying.floor.depth = 5;
    dying.dead = true;

    const fresh = startNewRun(99);
