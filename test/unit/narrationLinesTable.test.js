...
//
// Direct unit coverage for src/browser/narrationLines.js (Phase 25): every LINE_FOR
...
import { TONES, PRIORITY, ORACLE_ONLY, FEATURE_EVENTS, LINE_FOR, slotWord } from "../../src/browser/narrationLines.js";
...
test("every LINE_FOR builder survives a bare {type} call with a valid shape", () => {
  for (const [type, fn] of Object.entries(LINE_FOR)) {
    const r = fn({ type }, {});
    assert.ok(r, `LINE_FOR.${type} returned nothing`);
    assert.equal(typeof r.text, "string", `LINE_FOR.${type}.text should be a string`);
    assert.ok(r.text.trim().length > 0, `LINE_FOR.${type} returned empty text`);
    assert.ok(TONE_SET.has(r.tone), `LINE_FOR.${type}.tone "${r.tone}" is not a valid tone`);
    assert.ok(PRIORITY_VALUES.has(r.priority), `LINE_FOR.${type}.priority "${r.priority}" is not a valid priority`);
  }
});
...
test("reason-specific refusal text differs from the generic fallback", () => {
  const fallback = LINE_FOR.parleyRefused({ type: "parleyRefused", reason: "definitelyNotAReason" }).text;
  const ninja = LINE_FOR.parleyRefused({ type: "parleyRefused", reason: "ninja" }).text;
  assert.notEqual(ninja, fallback);

  const itemFallback = LINE_FOR.itemRejected({ type: "itemRejected", reason: "definitelyNotAReason" }).text;
  const woodsman = LINE_FOR.itemRejected({ type: "itemRejected", reason: "woodsman" }).text;
  assert.notEqual(woodsman, itemFallback);

  const equipFallback = LINE_FOR.equipRejected({ type: "equipRejected", reason: "definitelyNotAReason" }).text;
  const acrobat = LINE_FOR.equipRejected({ type: "equipRejected", reason: "acrobat" }).text;
  ...
  const jewelryFull = LINE_FOR.equipRejected({ type: "equipRejected", reason: "jewelryFull" }).text;
  assert.notEqual(jewelryFull, equipFallback);
  assert.equal(LINE_FOR.equipRejected({ type: "equipRejected", reason: "jewelryFull" }).tone, "block");
  const wrongSlot = LINE_FOR.equipRejected({ type: "equipRejected", reason: "wrongSlot" }).text;
  ...
  const castNotFought = LINE_FOR.castRefused({ type: "castRefused", reason: "notFought", spell: "Heal" }).text;
  ...
  assert.equal(
    LINE_FOR.itemEquipped({ item: { n: "Dagger" }, slot: "weapon" }).text,
    "Equipped: Dagger (weapon).",
  );

  // Phase 31 (CMB-02): a useRefused cooldown reason vs the generic fallback.
  const useCooldown = LINE_FOR.useRefused({ type: "useRefused", reason: "cooldown", item: { n: "Cloak" }, left: 12 }).text;
  ...
  const freeze = LINE_FOR.strikeRefused({ type: "strikeRefused", reason: "wizard", spell: "Freeze" }).text;
...
test("THEM-family builders return tone hurt/dodge and text starting with the foe name", () => {
  for (const t of THEM_TYPES) {
    const r = LINE_FOR[t]({ type: t, ...THEM_PAYLOAD });
...
test("locked wordings: struckByFoe", () => {
  assert.equal(LINE_FOR.struckByFoe({ type: "struckByFoe", name: "Dante", dmg: 6 }).text, "Dante hits you (6)");
  assert.equal(LINE_FOR.struckByFoe({ type: "struckByFoe", name: "Dante", dmg: 6, critical: true }).text, "Dante hits you (6) · CRIT");
  assert.ok(LINE_FOR.struckByFoe({ type: "struckByFoe", name: "Dante", dmg: 6, soldierCrit: true }).text.includes("· CRIT"));
  assert.equal(
    LINE_FOR.struckByFoe({ type: "struckByFoe", name: "Dante", dmg: 4, soaked: { hide: 2 } }).text,
...
test("locked wordings: memberStruck", () => {
  const r = LINE_FOR.memberStruck({ type: "memberStruck", name: "Dante", member: "Bram", dmg: 5 });
...
test("locked wordings: strikeMissed", () => {
  assert.equal(LINE_FOR.strikeMissed({ type: "strikeMissed", target: "Dante" }).text, "You miss Dante");
  assert.equal(
    LINE_FOR.strikeMissed({ type: "strikeMissed", target: "Dante", quip: "Wide. Impressively wide." }).text,
    "You miss Dante — Wide. Impressively wide.",
  );
  assert.ok(LINE_FOR.strikeMissed({ type: "strikeMissed", target: "Dante", untouchable: true }).text.includes("cannot"));
});
...
test("missing optional fields degrade gracefully, never throw", () => {
  assert.doesNotThrow(() => LINE_FOR.goldGained({ type: "goldGained" }));
  assert.equal(LINE_FOR.goldGained({ type: "goldGained", amount: 3 }).text, "+3 wilmst");
  const foeFled = LINE_FOR.foeFled({ type: "foeFled", name: "Rat" });
  assert.ok(foeFled.text.trim().length > 0);
  assert.equal(foeFled.tone, "hit");
  const encounterStarted = LINE_FOR.encounterStarted({ type: "encounterStarted" });
...
test("feature flag clauses render when present", () => {
  ...
  assert.ok(LINE_FOR.trapDisarmed({ type: "trapDisarmed" }).text.includes("Pilfer"));
  assert.ok(LINE_FOR.armorSoaked({ type: "armorSoaked", amount: 5, wear: 3, halved: true }).text.includes("Dwarven"));
  assert.ok(LINE_FOR.foeFled({ type: "foeFled", name: "Rat", reason: "knight" }).text.includes("Knight"));
});

// ─── Manifest sanity ─────────────────────────────────────────────────────────

test("FEATURE_EVENTS manifest sanity", () => {
  const missing = FEATURE_EVENTS.filter((t) => !(t in LINE_FOR));
  assert.deepEqual(missing, [], `every FEATURE_EVENTS entry must be a LINE_FOR key: ${missing.join(", ")}`);
  ...
  const overlap = [...ORACLE_ONLY].filter((t) => t in LINE_FOR);
  assert.deepEqual(overlap, [], `ORACLE_ONLY and LINE_FOR must be disjoint: ${overlap.join(", ")}`);
  assert.equal(FEATURE_EVENTS.length, new Set(FEATURE_EVENTS).size, "FEATURE_EVENTS must have no duplicates");
});

// ─── Purity ──────────────────────────────────────────────────────────────────

test("narrationLines.js source contains no Math.random / Date.now / document. / window. on any non-comment line", () => {
