...
| 4 | Humans | Herman | 36.00 | 1 | d6 (as L5) |  | 15.00 |  | 7.11 | 5.06 | 3.54 | 21.38 | 2.55 | 1.46 | over-tier |
| 5 | Humans | Herman | 36.00 | 1 | d6 (as L5) |  | 15.00 |  | 13.61 | 2.65 | 1.01 | 21.38 | 2.81 | 1.00 |  |
| 1 | Lair Beasts | Dog Face | 6.00 | 1 | 1d6+0 |  |  |  | 1.42 | 4.22 | 1.20 | 1.35 | 30.89 | 1.00 |  |
| 1 | Lair Beasts | Goblin | 4.00 | 1 | d6* |  |  |  | 1.42 | 2.81 | 0.80 | 1.35 | 30.89 | 1.00 |  |
| 1 | Lair Beasts | Hobgoblin | 5.00 | 1 | 1d6+1 |  |  |  | 1.42 | 3.51 | 1.00 | 1.65 | 25.27 | 1.22 |  |
| 1 | Lair Beasts | M&M | 3.00 | 1 | d6* |  |  |  | 1.42 | 2.11 | 0.60 | 1.35 | 30.89 | 1.00 |  |
...
#### Still flagged after the fixes (rule=dice) — ruled
...
**Every row above now carries its own `Disposition` cell (D-07) — nothing
below is silently skipped.** The user's ruling (52-CONTEXT.md, recorded at
plan approval, and re-confirmed in Plan 03's own execution instructions,
2026-09-20): Phase 52 fixes exactly the two cliffs (the general crit rule
and Herman's `sp.strikesAs`); **NO dice trims land in this phase.** Every
still-flagged row above gets an explicit per-row ruling instead of a trim:

- **`deep-tier` (21 cells — every tier-5 row above, including Herman T5):**
  the exact `Disposition` cell text on each of those 21 rows (quoted once,
  verbatim, above the table columns) — D-07 already allows this disposition
  for tier 5 without further review. Herman T5's own post-fix crit is 37
  (down from 100) — his tier-5 Endgame-band ruling is about the curve's
  height at tier 5 in general, not a residual cliff of his own; his cliff
  was fixed above.
- **`level-base` (14 cells — Herman T4 and every other tier's DEFAULT-d6
  ...
  Undead, Drake at the Wall band):** disposition **"curve height — a Phase
  54 dial (four-band retune), not a Phase 52 cliff"** — the tier's own
  `lvl^2 + 12` crit maximum (the tier's default d6 crit) trips the bar on
  its own; the row's OWN dice are not the cause. Herman T4's post-fix crit
  is also 37 (down from 82, the tier-5 default by rulebook design, since he
  `strikesAs` a level five) — his own cliff is fixed above; this level-base
  disposition is about the tier's default curve height, handed to Phase 54's
  TUNE-08 (per-creature decision), not a Phase 52 cliff.
- **`row-dice` (30 cells — Rast, Sterling, Drake T4, Frank, Primp, Craig,
  Pogo, Trachea, Blumble, Werebeast, Wolf):** disposition **"curve height —
  a Phase 54 dial (four-band retune), not a Phase 52 cliff"** — the row's
  OWN `sp.dmg` notation is what trips the flag; the `wouldBeTrim` column
  above shows the largest same-family notation that would clear the bar,
  handed to Phase 54's TUNE-08 (per-creature decision) with that
  `wouldBeTrim` notation as its input; no bestiary numbers change in
  Plan 03.
...
`Disposition` cell, satisfying D-07's letter (deep-tier/tier-5 rows) or its
spirit (level-base/row-dice rows, explicitly handed to Phase 54 rather than
trimmed mid-milestone).

**Handoff to Phase 54 (TUNE-08):** the 44 `level-base`/`row-dice` cells
above (Stink Bug, Ghost, Spectre, Craig, Drarl, Drudge, Bones, Floater,
Undead, Drake, Rast, Sterling, Frank, Primp, Pogo, Trachea, Blumble,
Werebeast, Wolf, Herman T4) are handed to Phase 54's per-creature
stays/moves/retune decision, with this table's `wouldBeTrim` column as the
starting input for each row's dice-notation option.
